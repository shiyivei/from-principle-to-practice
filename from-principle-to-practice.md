...
迭代器有三种类型：Iter，IterMut，IntoIter，对应所有权三种语义
...
```
cargo new process-macro --lib
```

在toml中加入下列内容，这样编译器才允许你使用#[proc_macro]相关的宏
...
一般不会使用数据库驱动直接连接数据库，而是使用ORM，Rust有diesel，还有sea-orm（支持异步）
...
使用schema 定义的文件可以用serde和serde-yaml读取
...
基于TCP的协议：分隔符是 `\r\n`,如FTP，但是gRPC使用消息长度做分割符号它基于http/2。混用的：Redis的RESP

复杂的如hTPP，header之间使用`\r\n`,header/body之间使用`\r\n\r\n`,hea会提供body的长度等

 `\r\n`适合协议报文是ASCII数据，通过长度分割，适合二进制数据。protobuf 是二进制，所以就在 payload 之前放一个长度，来作为 frame 的分隔

这个长度取什么大小呢？如果使用 2 个字节，那么 payload 最大是 64k；如果使用 4 个字节，payload 可以到 4G。一般的应用取 4 个字节就足够了。如果你想要更灵活些，也可以使用

# 10 生产环境中的项目

## 10.1 pre-commit hook脚本
...
和文档放在一起的单元测试，如：它可以使用cargo doc 直接打开，也可以使用cargo test直接运行
...
放在tests目录下，每个测试编成单独的二进制

集成测试的写法和单元测试其实很类似，只不过我们不需要再使用 #[cfg(test)] 来做条件编译

集成测试和你的 crate 用同样的条件编译，所以在集成测试里，无法使用单元测试中构建的辅助代码
...
使用github workflow来持续集成

```
.github/workflows 目录中创建一个 build.yaml 文件
然后定义触发器
定义构建步骤
配置测试环境
配置通知
...
处理代码格式，做基本的静态检查、单元测试和集成测试，以及生成文档，通过 GitHub Workflow 进行持续集成，可以使团队更加高效和协作，从而提高软件质量和交付速度
...
作为一门编译型语言，Rust支持条件编译，cargo book中有详细介绍
...
![img](https://static001.geekbang.org/resource/image/2a/ef/2a6c520f4402132a70bf44333664ccef.jpg?wh=2312x1379)

![img](https://static001.geekbang.org/resource/image/c7/3f/c78be3a1631658bbfa3a0693yy5d1a3f.jpg?wh=3129x1837)
